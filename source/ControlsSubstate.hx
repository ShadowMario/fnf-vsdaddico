	...
	override function update(elapsed:Float) {
		if(rebindingKey < 0) {
			...
			if (keyPressed > -1) {
				...
				var text = new InputItem(InputFormatter.getKeyName(controlArray[rebindingKey]), grpInputs[rebindingKey].offsetX, grpInputs[rebindingKey].offsetY);
				text.setPosition(grpInputs[rebindingKey].x, grpInputs[rebindingKey].y);
				text.sprTracker = grpInputs[rebindingKey].sprTracker;
				text.isAlt = grpInputs[rebindingKey].isAlt;
				grpInputs.remove(grpInputs[rebindingKey]);
				grpInputs.insert(rebindingKey, text);
				add(text);
				FlxG.sound.play(Paths.sound('confirmMenu'));
				rebindingKey = -1;
			}
