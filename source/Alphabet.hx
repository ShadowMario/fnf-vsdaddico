...
class Alphabet extends FlxSpriteGroup
{
	...
	public function addText()
	{
		...
		for (character in splitWords)
		{
			...
			if (isAlphabet || (!isBold && (isNumber || isSymbol)))
			{
				if (lastSprite != null)
				{
				...
				if (isBold)
				{
					if(!isNumber && !isSymbol && isAlphabet) letter.createBold(character);
				}
				else
				{
					if (isNumber)
	...
	public function timerCheck(?tmr:FlxTimer = null) {
		...
		if (isAlphabet || (!isBold && (isNumber || isSymbol)))
		{
			...
			if (isBold)
			{
				if(!isNumber && !isSymbol && isAlphabet) letter.createBold(splitWords[loopNum]);
			}
			else
			{
				if (isNumber)
